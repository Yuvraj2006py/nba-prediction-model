"""Collect Basketball Reference data for the most recent 3 seasons (2022-23, 2023-24, 2024-25).
...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler('logs/bball_ref_recent_collection.log'),
        logging.StreamHandler()
    ]
)
logger = logging.getLogger(__name__)

# Most recent 3 seasons to collect
RECENT_SEASONS = ['2022-23', '2023-24', '2024-25']

# Database path - using main database for recent seasons
MAIN_DB_PATH = Path(project_root) / "data" / "nba_predictions.db"
    ...
    for month in months_to_scrape:
        ...
        for row_idx, row in enumerate(rows):
            ...
            try:
                ...
                # Parse date: "Tue, Oct 22, 2022" or "Oct 22, 2022" or "2022-10-22"
                game_date = None
...
def collect_all_recent_seasons(collect_games: bool = True, replace_existing: bool = False):
    """Collect all recent seasons (2022-23, 2023-24, 2024-25)."""
    logger.info("=" * 70)
    logger.info("Basketball Reference Recent Seasons Collection (NO NBA API)")
    logger.info("=" * 70)
    logger.info(f"Seasons: {', '.join(RECENT_SEASONS)}")
    logger.info(f"Database: {MAIN_DB_PATH}")
    logger.info(f"Collect games first: {collect_games}")
    logger.info(f"Replace existing: {replace_existing}")
    logger.info("=" * 70)
    
    # Initialize database manager with main database
    ...
    for season in RECENT_SEASONS:
        try:
            ...
            if collect_games:
                ...
                logger.info(f"[OK] Collected {game_stats['games_stored']} games for {season}")
    ...
    # Final summary
    logger.info("\n" + "=" * 70)
    logger.info("All Recent Seasons Collection Complete")
    logger.info("=" * 70)
    ...
    logger.info(f"Database location: {MAIN_DB_PATH}")
    logger.info("=" * 70)
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description='Collect Basketball Reference data for recent seasons (2022-23, 2023-24, 2024-25) - NO NBA API',
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
Examples:
  # Collect games and stats for all recent seasons (from Basketball Reference only)
  python scripts/collect_recent_seasons_bball_ref.py
  
  # Skip game collection (assume games already exist in database)
  python scripts/collect_recent_seasons_bball_ref.py --no-collect-games
  
  # Replace existing data
  python scripts/collect_recent_seasons_bball_ref.py --replace
  
  # Collect specific season only
  python scripts/collect_recent_seasons_bball_ref.py --season 2022-23
  
  # Test on one month first
  python scripts/collect_recent_seasons_bball_ref.py --season 2022-23 --test-month october
        """
    )
    parser.add_argument('--season', type=str, choices=RECENT_SEASONS,
                       help='Season to collect (default: all recent seasons)')
